...
def get_mac_address() -> str:
    ...
    if res.returncode != 0 and not (res.stdout or res.stderr):
        raise ValueError("ifconfig failed with no output or error")

    output = (res.stdout or "").strip()
    if not output:
        raise ValueError("ifconfig produced empty output")
...
def get_cpu_arch() -> str:
    ...
    #TODO: output 구현
    res = _run(["uname", "-m"])
    arch = (res.stdout or "").strip()
    if not arch:
        raise ValueError("uname -m produced empty output")
    return arch
...
def get_pwd() -> str:
    ...
    #TODO: output 구현
    pwd = _run(["pwd"])
    pwd = (pwd.stdout or "").strip()
    if not pwd:
        raise ValueError("pwd produced empty output")
    return pwd
...
def check_uboot_aarch64(uboot_src: str) -> Dict[str, object]:
    ...
    # TODO: defconfig
    conf = _run(["make", "-C", uboot_src, "bcm2712_defconfig"])
    if conf.returncode != 0:
        raise ValueError(f"defconfig 실패\n{conf.stderr}")


    # TODO: build
    build = _run(["make", "-C", uboot_src, "-j4"])
