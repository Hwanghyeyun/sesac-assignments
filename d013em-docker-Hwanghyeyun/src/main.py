...
def task1_build_image() -> bool:
    ...
    # TODO: docker 빌드
    result = _run(["docker", "build", "-t", "msa:test", context_dir], check=True)

    # TODO: 빌드 실패 시 예외 발생
    if result.returncode != 0:
        raise RuntimeError("build 실패")


    # TODO: 이미지 확인, msa:test
    images = _run(["docker", "images", "--format", "{{.Repository}}:{{.Tag}}"], check=True)
...
def task2_run_and_check() -> bool:
    ...
    if run_res.returncode != 0:
        raise RuntimeError("docker run 실패")

    # TODO: docker 실행 여부 확인
    ps = _run(["docker", "ps", "--format", "{{.Names}}"], check=True)
...
def task3_nginx_http_200() -> bool:
    ...
    # TODO: 실행 실패 시 예외 발생
    if run_res.returncode != 0:
        raise RuntimeError("nginx 컨테이너 실행 실패")
...
def task4_env_pass_and_read() -> bool:
    ...
    var, value = "WHO", "MSA"
    res = _run(
        ["docker", "run", "--rm", "-e", f"{var}={value}", "alpine:3.19", "printenv", var],
        check=True,
    )

    # TODO: 실행 실패 시 예외 발생
    if res.returncode != 0:
        raise RuntimeError("docker run 환경변수 실패")
...
def task5_compose_demo() -> bool:
    ...
    # TODO: 실행 실패 시 예외 발생
    if up.returncode != 0:
        raise RuntimeError("docker compose 실행 실패")
